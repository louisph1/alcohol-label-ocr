...
Sometime it suffers from AI schizophrenia, mostly saying the GOVERNMENT WARNING is not capitalized when it is, but it still produces a good list of points for employees to look at. Having the thinking and resolution on highest helps somewhat. I consider minor annoyance this a fine trade-off for the more intelligent behavior of an LLM, since it's intended as a tool to help employees save time, it's better if it's oversensitive and marks things that don't exist rather than miss things that do. The model can easily be changed to a better one later as the technology always improves.
